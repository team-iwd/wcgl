...
var lightSpecularVertexShader = `#version 300 es
  ...
  void main() {
    ...
    vVec = normalize(u_eyePosition - v_worldPosition);
  }
`;
...
async function main() {
  ...
  let customModel = new Model();
  customModel.LoadModel(gl, 'https://raw.githubusercontent.com/raysan5/raylib/refs/heads/master/examples/models/resources/models/obj/well.obj')

  //--Texture Loading
  let checkerTexture = new Texture(gl);
  checkerTexture.LoadeTextureFromImage(gl,'../../resources/models/custom/custom_diffuse.png');
  ...
  let dullMat = new Material(5.0, 1.0); // 반사도가 낮은 머티리얼
  ...
  function drawScene()
  {
    ...
    shader.Bind(gl); //Uniform 설정에 필요하기 때문에 바인딩
    {
      ...
      mat4.translate(model, model, [-0.75, 0, 0]);
      ...
      customModel.RenderModel(gl, shader);
